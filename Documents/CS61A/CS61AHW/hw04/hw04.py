...
def totals_tree(m):
    ...
    total_t = [totals_tree(end(left(m))), totals_tree(end(right(m)))]
    return tree(total_weight(m), total_t)
...
def make_withdraw(balance, password):
    ...
    new_balance = balance
    typed_nums = []
    def corr_pw(withdraw_amount, typed_password):
        nonlocal new_balance, typed_nums
        if typed_password == password and len(typed_nums) < 3:
            if withdraw_amount <= new_balance:
                new_balance -= withdraw_amount
                return new_balance
            return 'Insufficient funds'
        elif len(typed_nums) == 3:
            return "Your account is locked. Attempts: " + str(typed_nums)
        else:
            typed_nums.append(typed_password)
